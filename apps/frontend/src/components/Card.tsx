...
export function PlayCard() {
  const gameModeData = [
    {
      icon: <img src="/lightning-bolt.png" className="inline-block mt-1 h-7 w-7" alt="online" />,
      title: 'Play Online',
    ...
    {
      icon: <img src="computer.png" className="inline-block mt-1 h-7 w-7" alt="computer" />,
      title: 'Computer',
      description: 'Challenge a bot from easy to master',
      disabled: true,
    },
    {
      icon: <img src="/friendship.png" className="inline-block mt-1 h-7 w-7" alt="friend" />,
      title: 'Play a Friend',
      description: 'Invite a Friend to a game of Chess',
      disabled: true,
    },
    {
      icon: <img src="/trophy.png" className="inline-block mt-1 h-7 w-7" alt="tournament" />,
      title: 'Tournaments',
      description: 'Join an Arena where anyone can Win',
      disabled: true,
    },
    {
      icon: <img src="/strategy.png" className="inline-block mt-1 h-7 w-7" alt="variants" />,
    ...
    <Card className="bg-transparent border-none">
      <CardHeader className="pb-3 text-center">
        <CardTitle className="font-semibold tracking-wide flex flex-col items-center justify-center">
          ...
          <img className="pl-1 w-1/2 mt-4" src="/chess.png" alt="chess" />
        </CardTitle>
        <CardDescription />
      </CardHeader>
      <CardContent className="grid gap-2 cursor-pointer mt-1">
        {gameModeData.map((data, index) => {
          return <GameModeComponent {...data} key={index} />;
        })}
      </CardContent>
    </Card>
  );
